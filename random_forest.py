...
def run_rf(X_mat, y_labels, params, regressor):
    '''
    ...
    model_paramlist = ['n_estimators',
                       ...
                       'warm_start']
    if not regressor:
        model_paramlist.append('class_weight')
    ...
    for loop_ind, train_ind_test_ind in enumerate(cross_val.split(X_mat, y_labels)):
        ...
        # train
        rf_model.fit(X_mat.iloc[train_ind], y_labels.iloc[train_ind])

        # predict
        y_pred_test = rf_model.predict(X_mat.iloc[test_ind])
        y_pred_train = rf_model.predict(X_mat.iloc[train_ind])
        ...
        storage_vars['feature_importance'].append(get_feature_importance(rf_model,X_mat))
...
if __name__ == "__main__":
    ...
    # load data
    # train and test
    metrics_results = run_rf(features, labels, parameters, model_type=='reg')

    # Gather, print, and plot feature importances
    feature_importances = average_feature_importance(metrics_results['feature_importance'])
    feature_importances.round(3).to_csv(outfiles['FEAT_IMPORT_FILE'],sep="\t",header=True,index=False)
    estimator_plots.barplot_feat_importance(feature_importances, 10, model_id, outfiles['FEATURE_FIG_FILE'])
